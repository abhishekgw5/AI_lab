...
import java.util.*;
//csp_crypto_arith
//assign unique numbers to all alphabets from 0 to 9
//such that word1 + word2 = word3
//here SEND + MORE = MONEY

public class csp_crypt_arith {
    ...
    static void solve(List<Character> letters, List<Integer> values, boolean[] visited, String[] words) {
        if (letters.size() == values.size()) {
            ...
            if (map.get(words[0].charAt(0)) == 0 || map.get(words[1].charAt(0)) == 0
                    || map.get(words[2].charAt(0)) == 0)
                return;
